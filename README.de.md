...
bunx playwright install chromium   # nur für PDF-Generierung nötig

# 2. Setup prüfen
bun run doctor                     # validiert alle Voraussetzungen
...
Standardmäßig vertraut `bun scan.mjs` (alias `bun run scan`) den Rückgaben der ATS-Feeds. Einige Unternehmen lassen alte Stellenanzeigen öffentlich verfügbar, obwohl Rollen bereits geschlossen sind. Mit `--verify` startet Playwright nach dem API-Lauf und entfernt abgelaufene Stellenanzeigen, bevor sie in `pipeline.md` landen:
