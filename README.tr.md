...
bunx playwright install chromium   # yalnızca PDF üretimi için gerekli

# 2. Kurulumu kontrol edin
bun run doctor                     # Tüm ön koşulları doğrular
...
bun agent-inbox.mjs add "..."   # bir sonraki oturum için isteği kuyruğa alır
...
Varsayılan olarak `bun scan.mjs` (`bun run scan` olarak da bilinir), her ATS beslemesinin döndürdüğüne güvenir. Bazı şirketler, rol kapandıktan sonra bile herkese açık API'lerinde eski ilanları bırakır, bu yüzden bu süresi dolmuş kayıtlar `pipeline.md`'ye sızabilir. API geçişinden sonra Playwright'ı başlatıp süresi dolmuş ilanları hatta girmeden önce düşürmek için `--verify` bayrağını geçin:

```bash
bun scan.mjs --verify          # sıfır token'lı keşif + Playwright canlılık kontrolü
```
...
```bash
bun run serve:dashboard   # TUI'yi başlat
bun run build:dashboard   # opsiyonel: bağımsız ikili dosyayı derle
```
