...
export function causeOf(stderr) {
  ...
  const afterCaret = caret >= 0 && lines[caret + 1] && !/^(Node\.js|Bun) v/.test(lines[caret + 1])
    ? lines[caret + 1]
    : null;
  return afterCaret
    ...
    || lines.find((s) => /^[\w$]*(Error|Exception):/i.test(s))
    ...
    || lines.filter((s) => !/^(Node\.js|Bun) v/.test(s)).slice(-1)[0]
    || '(no stderr)';
}
...
console.log('7c. a crashed writer\'s cause survives extraction');
{
  ...
  const SHAPES = [
    ['bare Error       ', `throw new Error('boom');`,                                    isBun ? 'error: boom' : 'Error: boom'],
    ['async bare Error ', `async function f(){ throw new Error('boom'); } await f();`,   isBun ? 'error: boom' : 'Error: boom'],
    ['non-Error reject ', `await Promise.reject('plain string');`,                       isBun ? 'error: plain string' : 'plain string'],
    ['undefined reject ', `await Promise.reject();`,                                     isBun ? 'error: undefined' : 'undefined'],
    ['typed Error      ', `throw new TypeError('typed');`,                               'TypeError: typed'],
    ['custom Error     ', `class LockTimeoutError extends Error{}\nthrow new LockTimeoutError('held 30s');`, isBun ? 'error: held 30s' : 'LockTimeoutError: held 30s'],
  ];
  ...
  const lost = SHAPES
    .filter(([, src]) => /^(Node\.js|Bun) v/.test(SHIPPED(shape('ctl', src))))
    .map(([label]) => label.trim());
  check('negative control: the previous extractor lost the bare-Error shapes',
    lost.length >= 4 && lost.every((l) => /bare Error|reject|custom Error/.test(l)),
    `lost=[${lost.join(', ')}]`);
}
...
try {
  await withPipelineLock(target, () => {
    const enter = process.versions.bun
    ...
    const exit = process.versions.bun
      ? BigInt(Math.round((performance.timeOrigin + performance.now()) * 1e6))
      : process.hrtime.bigint();
    writeFileSync(out, JSON.stringify({ id, enter: enter.toString(), exit: exit.toString() }));
  });
} catch (err) {
