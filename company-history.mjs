...
const USAGE = `Usage:
  ...
  bun company-history.mjs --summary --emit-signal  # human-readable cards, then RFC #1506 schema-v1 no-response-friction
  ...
  for (const row of Array.isArray(rows) ? rows : []) {
    ...
    if (normalized === 'applied') {
      ...
      facts.push({
        ...
        clearInstruction: `if they actually responded, bun set-status.mjs --row ${row.num} <state> --on <response-date> clears this`,
      });
...
export function renderSummary(result) {
  ...
  if (aged.length > 0) {
    lines.push(`  ${aged.length} aged-Applied row(s) look silent — confirm real or update (bun set-status.mjs --row <num> <state> --on <response-date>).`);
    lines.push('');
  }
