...
3. Say (anchored, so Cowork's own generic setup doesn't hijack the phrase "set me up"):
   > *"This folder contains career-ops. Read AGENTS.md, run its startup check (`bun doctor.mjs --json`) and walk me through career-ops onboarding based on its output."*
