...
```bash
bun plugins.mjs list          # what's installed + its trust badge
bun plugins.mjs available     # bundled + community plugins we've approved
bun plugins.mjs add <name>    # install an approved community plugin
bun plugins.mjs enable <id>   # show the capability card (then add --confirm)
bun plugins.mjs skill <id>    # print a plugin's how-to (if it ships one)
```

Two gates must both be satisfied for a plugin to run: it must be **enabled**
(`bun plugins.mjs enable <id> --confirm`, which records your consent) **and** its
keys must be in your `.env`. `bun doctor.mjs` shows what's missing.
...
asks you to review + `bun plugins.mjs trust <id>` to re-pin (tamper detection).

## Writing a plugin

```bash
bun plugins.mjs new my-plugin     # scaffolds plugins.local/my-plugin/
```
...
3. Open a **registry PR** (the `?template=plugin-registry.md` template — the
   ...
   `bun plugins.mjs add <name>` and your plugin ships to them via the normal
   update.
...
3. Once approved + pinned, anyone who runs `bun plugins.mjs add career-ops-plugin-<id>`
   installs your version, and the engine gives **your maintained successor
   precedence over the bundled reference** of the same id. `bun plugins.mjs available`
