...
const USAGE = `Usage:
  bun doctor.mjs                    # run the setup diagnostic
  bun doctor.mjs --json             # machine-readable onboarding state
  bun doctor.mjs --strict           # also probe portals.yml entries (network)
  bun doctor.mjs --target <path>    # diagnose another career-ops checkout
  bun doctor.mjs --cli <name>       # check a specific CLI's integration
  bun doctor.mjs --help             # show this message
...
// Opt-in so the default `bun doctor` stays fast and fully offline.
...
async function checkPlaywright() {
  ...
  let browser;
  try {
    browser = await chromium.launch({ headless: true, timeout: 3000 });
