 ...
 *
 * Usage:
 *   bun generate-latex.mjs <input.tex> [output.pdf]
 *   bun generate-latex.mjs <input.tex> [output.pdf] --compile-only
 *
...
async function main() {
  ...
  if (!inputPath) {
    console.error('Usage: bun generate-latex.mjs <input.tex> [output.pdf] [--compile-only]');
    process.exit(1);
  }
