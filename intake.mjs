...
//
// Usage:
//   bun intake.mjs                 # scan + extract, JSON to stdout
//   bun intake.mjs --summary       # human-readable table instead of JSON
//   bun intake.mjs --text <path>   # full extracted text of one source
//   bun intake.mjs --commit <path …>
//                                   # record the confirmed sources as ingested
//                                   # (run only after the user confirmed)
//   bun intake.mjs --commit --all  # record every source with new material —
//                                   # only when the user merged all of them
//   bun intake.mjs --self-test     # pure-function self-test, no filesystem
...
function main() {
  ...
  if (textIdx !== -1) {
    ...
    if (!target) { console.error('Usage: bun intake.mjs --text <path relative to documents/>'); process.exit(1); }
  ...
  if (commitIdx !== -1) {
    ...
    if (!all && only.length === 0) {
      console.error(
        'Nothing to commit. Pass the confirmed sources — bun intake.mjs --commit <path> [<path> …] — '
        + 'or bun intake.mjs --commit --all when every source with new material was merged. '
