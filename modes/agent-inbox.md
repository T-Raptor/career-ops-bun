...
bun agent-inbox.mjs add "draft a follow-up for application #7"
bun agent-inbox.mjs add "run a scan and triage anything new"
```

## Inspect / resolve

```bash
bun agent-inbox.mjs list            # pending items
bun agent-inbox.mjs list --all      # include resolved items
...
3. After each, mark it `[x]` and append `→ result: <one line>` — either by hand
   or with `bun agent-inbox.mjs resolve <n> --result "..."`.
