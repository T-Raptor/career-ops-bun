...
4. **For each pending URL**:
   ...
   c. Reserve the next REPORT_NUM atomically: `bun reserve-report-num.mjs` (release with `--release {num}` after the worker writes the report; stale sentinels are GC'd automatically)
   d. Execute via Bash:
