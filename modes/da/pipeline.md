...
2. **For hver afventende URL**:
   a. Reservér det næste fortløbende `REPORT_NUM` atomisk ved at køre `bun reserve-report-num.mjs` (og frigiv sentinel'en ved at køre `bun reserve-report-num.mjs --release <num>`, når rapporten er skrevet)
...
1. **Playwright (foretrukket):** `browser_navigate` + `browser_snapshot`. Fungerer med alle SPAs.
   - **Tilvalg — CLI-udtrækker (`scan.extractor: cli` i `config/profile.yml`):** kør `bun browser-extract.mjs <url>` (`--mode jd`) i stedet — kompakt `{ "url", "title", "text" }`, færre tokens (afhænger af jobportalen). **Fald stille tilbage** til `browser_navigate` + `browser_snapshot` ved fejl eller hvis den mangler.
...
1. Kør `bun reserve-report-num.mjs` for at reservere det næste fortløbende nummer atomisk (stdout returnerer `{###}`).
2. Skriv rapporten med dette nummer.
3. Frigiv sentinel'en ved at køre `bun reserve-report-num.mjs --release {###}`, når rapporten er skrevet.
