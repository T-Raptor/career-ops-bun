...
2. **Para cada URL pendiente**:
   a. Reservar el siguiente `REPORT_NUM` secuencial de forma atómica ejecutando `bun reserve-report-num.mjs` (y liberar el sentinel ejecutando `bun reserve-report-num.mjs --release <num>` una vez escrito el report)
...
1. **Playwright (preferido):** `browser_navigate` + `browser_snapshot`. Funciona con todas las SPAs.
   - **Opcional — extractor CLI (`scan.extractor: cli` en `config/profile.yml`):** ejecuta `bun browser-extract.mjs <url>` (`--mode jd`) en su lugar — `{ "url", "title", "text" }` compacto, menos tokens (según el portal). **Recurre en silencio** a `browser_navigate` + `browser_snapshot` si falla o no está disponible.
...
1. Ejecutar `bun reserve-report-num.mjs` para reservar el siguiente número secuencial de forma atómica (stdout devuelve `{###}`).
2. Escribir el report con ese número.
3. Liberar el sentinel ejecutando `bun reserve-report-num.mjs --release {###}` una vez escrito el report.
