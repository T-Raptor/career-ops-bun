...
2. **Pour chaque URL en attente** :
   a. Reserver le prochain `REPORT_NUM` sequentiel de maniere atomique en executant `bun reserve-report-num.mjs` (et liberer le sentinel en executant `bun reserve-report-num.mjs --release <num>` une fois le rapport ecrit)
...
1. **Playwright (prefere) :** `browser_navigate` + `browser_snapshot`. Fonctionne avec toutes les SPAs.
   - **Option — extracteur CLI (`scan.extractor: cli` dans `config/profile.yml`) :** exécutez plutôt `bun browser-extract.mjs <url>` (`--mode jd`) — `{ "url", "title", "text" }` compact, moins de tokens (selon le site). **Repli silencieux** vers `browser_navigate` + `browser_snapshot` en cas d'erreur ou d'absence.
...
1. Executer `bun reserve-report-num.mjs` pour reserver le prochain numero sequentiel de maniere atomique (stdout renvoie `{###}`).
2. Ecrire le rapport avec ce numero.
3. Liberer le sentinel en executant `bun reserve-report-num.mjs --release {###}` une fois le rapport ecrit.
