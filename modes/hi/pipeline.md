...
2. **हर pending URL के लिए**:
   a. `bun reserve-report-num.mjs` run करके atomically अगला `REPORT_NUM` reserve करें (और report लिखने के बाद `bun reserve-report-num.mjs --release <num>` run करके sentinel release करें)
...
1. **Playwright (preferred):** `browser_navigate` + `browser_snapshot`। सभी SPAs के साथ काम करता है।
   - **वैकल्पिक — CLI एक्सट्रैक्टर (`config/profile.yml` में `scan.extractor: cli`):** इसके बजाय `bun browser-extract.mjs <url>` (`--mode jd`) चलाएँ — कॉम्पैक्ट `{ "url", "title", "text" }`, कम टोकन (पोर्टल पर निर्भर)। त्रुटि या अनुपलब्ध होने पर **चुपचाप** `browser_navigate` + `browser_snapshot` पर लौटें।
...
1. `bun reserve-report-num.mjs` run करें atomically अगला sequential number reserve करने के लिए (stdout `{###}` return करता है)।
2. उस number से report लिखें।
3. Report लिखने के बाद `bun reserve-report-num.mjs --release {###}` run करें sentinel release करने के लिए।
