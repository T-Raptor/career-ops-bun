...
2. **Untuk setiap URL yang menunggu**:
   a. Pesan `REPORT_NUM` urut berikutnya secara atomik dengan menjalankan `bun reserve-report-num.mjs` (dan lepas sentinel dengan menjalankan `bun reserve-report-num.mjs --release <num>` setelah report ditulis)
...
1. **Playwright (diutamakan):** `browser_navigate` + `browser_snapshot`. Berfungsi dengan semua SPA.
   - **Opsional — ekstraktor CLI (`scan.extractor: cli` di `config/profile.yml`):** jalankan `bun browser-extract.mjs <url>` (`--mode jd`) sebagai gantinya — `{ "url", "title", "text" }` ringkas, lebih sedikit token (bergantung portal). **Kembali diam-diam** ke `browser_navigate` + `browser_snapshot` jika gagal atau tidak ada.
...
1. Jalankan `bun reserve-report-num.mjs` untuk memesan nomor urut berikutnya secara atomik (stdout mengembalikan `{###}`).
2. Tulis report dengan nomor tersebut.
3. Lepas sentinel dengan menjalankan `bun reserve-report-num.mjs --release {###}` setelah report ditulis.
