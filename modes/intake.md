...
```bash
bun intake.mjs            # JSON: per-source status + preview
```
...
```bash
bun intake.mjs --text <path-relative-to-documents/>
```
...
```bash
bun intake.mjs --commit <path> [<path> …]   # the confirmed sources
bun intake.mjs --commit --all               # only if ALL were merged
```
...
3. Verify: `bun doctor.mjs` should report the profile prerequisites
   satisfied.
