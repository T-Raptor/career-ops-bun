...
2. **Per ogni URL in attesa**:
   a. Riservare il prossimo `REPORT_NUM` sequenziale eseguendo `bun reserve-report-num.mjs` (liberare la prenotazione con `bun reserve-report-num.mjs --release <num>` una volta scritto il report)
...
1. **Playwright (preferito):** `browser_navigate` + `browser_snapshot`. Funziona con tutte le Single Page Application (SPA).
   - **Opzionale — estrattore CLI (`scan.extractor: cli` in `config/profile.yml`):** esegui invece `bun browser-extract.mjs <url>` (`--mode jd`) — `{ "url", "title", "text" }` compatto, meno token (dipende dal portale). **Ripiega in silenzio** su `browser_navigate` + `browser_snapshot` in caso di errore o assenza.
...
1. Eseguire `bun reserve-report-num.mjs` per riservare in modo atomico il prossimo numero sequenziale (restituisce `{###}` su stdout).
2. Scrivere il report con questo numero.
3. Liberare la prenotazione con `bun reserve-report-num.mjs --release {###}` una volta scritto il report.
