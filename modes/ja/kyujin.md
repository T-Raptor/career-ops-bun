...
- `{###}` = next sequential number（3 digits, zero-padded）。競合を防ぐため、必ず `bun reserve-report-num.mjs` を実行して番号を claim し（stdout returns `{###}`）、report を write してから `bun reserve-report-num.mjs --release {###}` を実行して sentinel を release する。
