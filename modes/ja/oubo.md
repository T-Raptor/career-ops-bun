...
1. 正規 CLI で status を "Applied" に update: `bun set-status.mjs <report#> Applied`（`applications.md` のテーブルを手動編集しない）
