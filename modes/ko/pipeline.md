...
2. **각 대기 URL에 대해**:
   a. `bun reserve-report-num.mjs`를 실행해 다음 sequential `REPORT_NUM`을 atomic하게 예약합니다(report 작성 후 `bun reserve-report-num.mjs --release <num>`으로 sentinel 해제).
...
1. **Playwright (preferred):** `browser_navigate` + `browser_snapshot`. 모든 SPA에 대응합니다.
   - **선택 — CLI 추출기 (`config/profile.yml`의 `scan.extractor: cli`):** 대신 `bun browser-extract.mjs <url>`(`--mode jd`) 실행 — 간결한 `{ "url", "title", "text" }`, 더 적은 토큰(포털에 따라 다름). 오류나 부재 시 **조용히** `browser_navigate` + `browser_snapshot`로 폴백.
...
1. `bun reserve-report-num.mjs`를 실행해 다음 sequential number를 예약합니다(stdout이 `{###}` 반환).
2. 해당 번호로 report를 작성합니다.
3. report 작성 후 `bun reserve-report-num.mjs --release {###}`를 실행해 sentinel을 해제합니다.
