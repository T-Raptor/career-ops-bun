...
2. Run `bun check-liveness.mjs --file <tmpfile>` (add `--throttle` for large batches to stay under WAF rate limits; it's pure Playwright, zero Claude tokens). The checker prints a per-URL verdict and exits non-zero if any are expired/uncertain.
...
2. **For each surviving pending URL**:
   ...
   d. Claim the next sequential `REPORT_NUM` atomically by running `bun reserve-report-num.mjs` (and release the sentinel using `bun reserve-report-num.mjs --release <num>` after the report is written)
...
- `| rank: {score}/5 — {reason}` — an **opt-in** LLM relevance annotation written
  only by `bun rank-pipeline.mjs`, never by a scan. The score is 0–5 to one
...
1. Run `bun reserve-report-num.mjs` to claim the next sequential number (stdout returns `{###}`).
2. Write the report file using that number.
3. Release the sentinel by running `bun reserve-report-num.mjs --release {###}` once the report is written.
