...
2. **Dla każdego oczekującego URL-a**:
   a. Zarezerwuj następny kolejny `REPORT_NUM` atomowo, uruchamiając `bun reserve-report-num.mjs` (i zwolnij sentinel, uruchamiając `bun reserve-report-num.mjs --release <num>`, gdy report zostanie zapisany)
...
1. **Playwright (preferowane):** `browser_navigate` + `browser_snapshot`. Działa ze wszystkimi SPA.
   - **Opcjonalnie — ekstraktor CLI (`scan.extractor: cli` w `config/profile.yml`):** zamiast tego uruchom `bun browser-extract.mjs <url>` (`--mode jd`) — zwięzłe `{ "url", "title", "text" }`, mniej tokenów (zależnie od portalu). **Cichy powrót** do `browser_navigate` + `browser_snapshot` przy błędzie lub braku.
...
1. Uruchom `bun reserve-report-num.mjs`, aby zarezerwować następny kolejny numer atomowo (stdout zwraca `{###}`).
2. Zapisz report z tym numerem.
3. Zwolnij sentinel, uruchamiając `bun reserve-report-num.mjs --release {###}`, gdy report zostanie zapisany.
