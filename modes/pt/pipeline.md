...
2. **Para cada URL pendente**:
   a. Reservar o próximo `REPORT_NUM` sequencial de forma atômica executando `bun reserve-report-num.mjs` (e liberar o sentinel executando `bun reserve-report-num.mjs --release <num>` após a gravação do relatório)
...
1. **Playwright (preferido):** `browser_navigate` + `browser_snapshot`. Funciona com todas as SPAs.
   - **Opcional — extrator CLI (`scan.extractor: cli` em `config/profile.yml`):** execute `bun browser-extract.mjs <url>` (`--mode jd`) em vez disso — `{ "url", "title", "text" }` compacto, menos tokens (depende do portal). **Recorre em silêncio** a `browser_navigate` + `browser_snapshot` em caso de erro ou ausência.
...
1. Executar `bun reserve-report-num.mjs` para reservar o próximo número sequencial de forma atômica (a saída retornará `{###}`).
2. Gravar o arquivo de relatório usando esse número.
3. Liberar o sentinel executando `bun reserve-report-num.mjs --release {###}` após a gravação do relatório.
