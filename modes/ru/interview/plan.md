...
9. **Prior stated compensation** — если известен номер трекера, выполни `bun salary-gap.mjs --stated-for <tracker#>` (без токенов). Любое предыдущее наблюдение `stated` — это сумма, которую кандидат уже озвучил ранее, на конкретного интервьюера, в конкретном раунде; подставь его в Step 4, чтобы кандидат оставался последовательным и не renegotiating случайно.
