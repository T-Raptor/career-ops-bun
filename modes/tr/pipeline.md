...
2. **Her bekleyen URL için:**
   a. Sıradaki `REPORT_NUM` değerini atomik olarak rezerve etmek üzere `bun reserve-report-num.mjs` komutunu çalıştır (ve rapor yazıldıktan sonra `bun reserve-report-num.mjs --release <num>` çalıştırarak sentineli serbest bırak)
...
1. **Playwright (tercih edilen):** `browser_navigate` + `browser_snapshot` — tüm SPA'larla çalışır.
   - **İsteğe bağlı — CLI çıkarıcı (`config/profile.yml` içinde `scan.extractor: cli`):** bunun yerine `bun browser-extract.mjs <url>` (`--mode jd`) çalıştır — kompakt `{ "url", "title", "text" }`, daha az token (portala bağlı). Hata veya eksiklik durumunda **sessizce** `browser_navigate` + `browser_snapshot`'a geri dön.
...
1. Sıradaki rapor numarasını atomik olarak rezerve etmek için `bun reserve-report-num.mjs` komutunu çalıştır (standart çıktı `{###}` değerini döndürür).
2. Bu numarayı kullanarak rapor dosyasını yaz.
3. Rapor yazıldıktan sonra `bun reserve-report-num.mjs --release {###}` komutunu çalıştırarak sentineli serbest bırak.
