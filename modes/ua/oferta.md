...
- `{###}` = наступний порядковий номер (3 цифри, zero-padded). Щоб виділити цей номер атомарно та уникнути станів гонки, ви ПОВИННІ виконати `bun reserve-report-num.mjs` для резервування номера (stdout поверне `{###}`), записати звіт, а потім виконати `bun reserve-report-num.mjs --release {###}` для звільнення маркера (sentinel).
...
Потім виконати `bun merge-tracker.mjs` для злиття в `data/applications.md`.
