...
2. **Для кожного URL**:
   a. Зарезервувати наступний `REPORT_NUM` атомарно, запустивши `bun reserve-report-num.mjs` (та звільнити маркер, запустивши `bun reserve-report-num.mjs --release <num>` після запису звіту)
...
1. **Playwright (переважно):** `browser_navigate` + `browser_snapshot`. Працює з усіма SPA.
   - **Опційно — CLI-екстрактор (`scan.extractor: cli` у `config/profile.yml`):** натомість запустіть `bun browser-extract.mjs <url>` (`--mode jd`) — компактний `{ "url", "title", "text" }`, менше токенів (залежно від порталу). У разі помилки чи відсутності **тихо** відкотіться до `browser_navigate` + `browser_snapshot`.
...
1. Запустити `bun reserve-report-num.mjs` для атомарного резервування наступного порядкового номера (stdout поверне `{###}`).
2. Записати файл звіту, використовуючи цей номер.
3. Звільнити маркер, запустивши `bun reserve-report-num.mjs --release {###}` після запису звіту.
