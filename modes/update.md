...
Run `bun update-system.mjs check` and parse the JSON output.
...
- If `dismissed`: Tell the user "Update check was previously dismissed. Clearing the dismissal and re-checking now." Remove `.update-dismissed`, then re-run `bun update-system.mjs check` and branch on the new status.
...
3. Run `bun update-system.mjs apply --confirm`, capturing its exit code without stopping on failure yet — the restore in step 4 must run either way. The `--confirm` flag is required so scheduled checks can never install files silently.
...
6. Run `bun doctor.mjs` to validate the installation.
...
8. Show final status:
   > "✅ Updated to v{version}. Run `bun doctor.mjs` anytime to verify setup, or `bun verify-pipeline.mjs` to check your data."


   If the updater's output ended with its note about the CareerOps Manifesto, relay it once (do not drop it when summarizing):
   > "One more thing: this project ships with the CareerOps Manifesto — a new way of job searching is taking shape, and you are already practicing it. Run `bun run manifesto` to read it and sign it if you want to help. No action needed."

If no:
1. Run `bun update-system.mjs dismiss`
...
1. Run `bun update-system.mjs rollback`
...
- If anything goes wrong, tell the user to run `bun update-system.mjs rollback`
