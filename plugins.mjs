 ...
 *
 *   bun plugins.mjs list                       # discovered plugins + status
 *   bun plugins.mjs run <id> [hook] [args…]    # run one hook of one plugin
 *   bun plugins.mjs run gmail                  # ingest (the plugin's only hook)
 *   bun plugins.mjs run notion search "staff platform engineer"
 *   bun plugins.mjs run notion export [--dry-run]
 *
 * Provider plugins are NOT run here — they ride `bun scan.mjs` via a
 * `provider: <id>` entry in portals.yml. Keeping ingest/search/notify/export
 * behind this explicit CLI is deliberate: a plain `bun scan.mjs` never silently
...
async function cmdList() {
  ...
  console.log('\nEnable in config/plugins.yml, add keys to .env, then `bun plugins.mjs run <id>`.');
}
...
async function cmdRun(args) {
  ...
  if (!id) { console.error('Usage: bun plugins.mjs run <id> [hook] [args…] [--dry-run]'); process.exit(1); }

  const cfg = await loadPluginConfig(ROOT);
  const manifest = discoverPlugins(pluginRoots(ROOT), resolveSuccessorIds(ROOT)).find(m => m.id === id);
  if (!manifest) { console.error(`Unknown plugin "${id}". Run \`bun plugins.mjs list\`.`); process.exit(1); }
  ...
  if (!hook) {
    ...
    else { console.error(`Plugin "${id}" exposes multiple hooks (${runnable.join(', ')}). Specify one: bun plugins.mjs run ${id} <hook>`); process.exit(1); }
  }
  ...
  if (hook === 'ingest' || hook === 'search') {
    ...
    if (hook === 'search' && !payload) { console.error(`search needs a query: bun plugins.mjs run ${id} search "<query>"`); process.exit(1); }
...
async function cmdAvailable() {
  ...
  if (reg.plugins.length === 0) {
    ...
    console.log('\nInstall:  bun plugins.mjs add <name>');
  }
}
...
function cmdEnable(args) {
  ...
  if (!id) { console.error('Usage: bun plugins.mjs enable <id> [--confirm]'); process.exit(1); }
  const m = findManifest(id);
  if (!m) { console.error(`Unknown plugin "${id}". Run \`bun plugins.mjs list\`.`); process.exit(1); }
...
function cmdRemove(args) {
  const id = args[0];
  if (!id) { console.error('Usage: bun plugins.mjs remove <id>'); process.exit(1); }
...
function cmdNew(args) {
  const name = args[0];
  if (!name) { console.error('Usage: bun plugins.mjs new <name>'); process.exit(1); }
  let dest;
  ...
  console.log(`    A) develop locally:  bun plugins.mjs enable ${name}`);
...
async function cmdAdd(args) {
  ...
  const confirm = args.includes('--confirm');
  if (!target) { console.error('Usage: bun plugins.mjs add <name|owner/repo> [--sha <commit>] [--confirm]'); process.exit(1); }
  ...
  else console.log(`\n  To enable it (grants the above), run:  bun plugins.mjs enable ${installed.id} --confirm`);
}
...
async function main() {
  ...
  switch (cmd) {
    ...
    default:
      console.error('Usage: bun plugins.mjs [list | available | run <id> [hook] | skill <id> | new <name> | add <name|owner/repo> [--sha <c>] [--confirm] | enable <id> [--confirm] | trust <id> | remove <id>]');
      process.exit(1);
  }
}
