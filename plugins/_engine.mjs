 ...
 *    on every run, and that import must be free — so a plain `bun scan.mjs` with
...
export function lockGate(manifest, root) {
  ...
  switch (d.status) {
    ...
    case 'drift-nobump':
      ...
      warnSkip(manifest.id, `files changed since you trusted it without a version bump — possible tampering (${d.changedFiles.slice(0, 5).join(', ')}). Review, then \`bun plugins.mjs trust ${manifest.id}\``);
      return { load: false };
    case 'surface-widened':
      warnSkip(manifest.id, `capability surface expanded since you consented (${[...d.addedHosts, ...d.addedEnv].join(', ')}${manifest.allowsLocalhost ? ', localhost' : ''}) — re-consent: \`bun plugins.mjs enable ${manifest.id}\``);
      return { load: false };
...
function inactiveProviderStub(id, reason) {
  return {
    id,
    detect: () => null,
    fetch: async () => { throw new Error(`plugin "${id}" inactive: ${reason}. Run \`bun doctor.mjs\` for setup.`); },
  };
}
  ...
  try {
    ...
    for (const manifest of configuredOn) {
      ...
      if (!lockGate(manifest, root).load) {
        providersMap.set(manifest.id, inactiveProviderStub(manifest.id, 'integrity/consent check failed — see ⚠️ above; run `bun plugins.mjs trust ' + manifest.id + '` or `enable ' + manifest.id + '`'));
        continue;
      }
