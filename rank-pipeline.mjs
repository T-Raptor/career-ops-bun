 ...
 *
 * Usage:
 *   bun rank-pipeline.mjs                     # rank up to --limit pending entries
 *   bun rank-pipeline.mjs --limit 10
 *   bun rank-pipeline.mjs --cli codex         # override CLI auto-detection
 *   bun rank-pipeline.mjs --model <name>      # passed through when the CLI takes one
 *   bun rank-pipeline.mjs --dry-run           # print what would be written
 *   bun rank-pipeline.mjs --self-test         # in-memory suite; spawns no subprocess
 */
...
const USAGE = `
  ...
  bun rank-pipeline.mjs [--limit N] [--cli <name>] [--model <name>] [--dry-run]
