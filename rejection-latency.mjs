 ...
 * opt-in guarantee).
 *
 * Run: bun rejection-latency.mjs             (JSON to stdout)
 *      bun rejection-latency.mjs --summary   (human-readable table)
 *      bun rejection-latency.mjs --courtesy-days 21
 *      bun rejection-latency.mjs --today 2026-07-17         (deterministic runs/tests)
 *      bun rejection-latency.mjs --file path/to/active-interviews.md
 *      bun rejection-latency.mjs --tracker path/to/applications.md
 *      bun rejection-latency.mjs --self-test
 *
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  const USAGE = `Usage:
  bun rejection-latency.mjs                        # JSON report
  bun rejection-latency.mjs --summary              # human-readable table
  bun rejection-latency.mjs --file <path>          # a different active-interviews.md
  bun rejection-latency.mjs --tracker <path>       # a different applications.md
  bun rejection-latency.mjs --courtesy-days <N>    # override the courtesy threshold (default ${DEFAULT_COURTESY_DAYS})
  bun rejection-latency.mjs --today <YYYY-MM-DD>   # evaluate as of a fixed date
  bun rejection-latency.mjs --self-test            # run the built-in fixtures
  bun rejection-latency.mjs --help                 # show this message
