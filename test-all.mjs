...
try {
  ...
  try {
    ...
    await appendToPipeline([{ url: 'https://jobs.example.com/1', company: 'Acme', title: 'Engineer' }], { pipelinePath: fixturePipelinePath });
...
try {
  ...
  if (!existsSync(webLib)) {
    ...
    if (missing.length > 0) {
      ...
      } else if (run(NODE, process.versions.bun ? ['test', ...webUnits] : ['--test', ...webUnits], { timeout: 180000 }) !== null) {
...
try {
  ...
  const pkg = JSON.parse(readFile('package.json'));
  if (pkg.scripts && (pkg.scripts['jd-archive'] === 'node check-jd-archive.mjs' || pkg.scripts['jd-archive'] === 'bun check-jd-archive.mjs')) {
