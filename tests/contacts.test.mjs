 ...
 *
 * Run: bun test-all.mjs --only contacts
...
const spawnContacts = (...argv) => spawnSync(process.execPath, [scriptPath, ...argv], { encoding: 'utf-8', timeout: 10000 });
...
try {
  ...
  const eqFormOut = execFileSync(process.execPath, [tmpScript, '--vcf=output/eq-form.vcf'], { encoding: 'utf-8', timeout: 10000, cwd: tmpRoot });
