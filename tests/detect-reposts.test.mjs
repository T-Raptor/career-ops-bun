 ...
 *
 * Run: bun test-all.mjs --only detect-reposts
...
const minSpanOut = execFileSync(process.execPath, [scriptPath, '--min-span', '3'], {
  encoding: 'utf-8', timeout: 10000,
  cwd: dirname(scriptPath),
});
...
const spanEqOut = execFileSync(process.execPath, [scriptPath, '--min-span=30'], {
  encoding: 'utf-8', timeout: 10000,
  cwd: dirname(scriptPath),
});
eq('--min-span=30 is honoured (not silently dropped)', JSON.parse(spanEqOut).metadata.minSpanDays, 30);

const spanSpaceOut = execFileSync(process.execPath, [scriptPath, '--min-span', '30'], {
  encoding: 'utf-8', timeout: 10000,
  cwd: dirname(scriptPath),
});
eq('--min-span 30 is honoured', JSON.parse(spanSpaceOut).metadata.minSpanDays, 30);

const badSpanOut = execFileSync(process.execPath, [scriptPath, '--min-span', 'abc'], {
  encoding: 'utf-8', timeout: 10000,
  cwd: dirname(scriptPath),
});
...
for (const [flag, value, field, expected, label] of flagFallbackCases) {
  const out = execFileSync(process.execPath, [scriptPath, flag, value], {
    encoding: 'utf-8', timeout: 10000,
    cwd: dirname(scriptPath),
  });
...
const emptySpanOut = execFileSync(process.execPath, [scriptPath, '--min-span='], {
  encoding: 'utf-8', timeout: 10000,
  cwd: dirname(scriptPath),
});
...
const zeroSpanOut = execFileSync(process.execPath, [scriptPath, '--min-span', '0'], {
  encoding: 'utf-8', timeout: 10000,
  cwd: dirname(scriptPath),
});
...
const metaSanity = JSON.parse(execFileSync(process.execPath, [scriptPath, '--min-span', '9'.repeat(400)], {
  encoding: 'utf-8', timeout: 10000,
  cwd: dirname(scriptPath),
})).metadata;
