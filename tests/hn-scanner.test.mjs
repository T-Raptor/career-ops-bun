...
test('Hacker News AI Extraction Logic - should extract valid data from a standard HN post', async () => {
...
test('Hacker News AI Extraction Logic - should return null for malformed YAML output', async () => {
  const res = await extractWithAI('MALFORMED data', mockModel);
  assert.strictEqual(res, null);
});

test('Hacker News AI Extraction Logic - should return null for non-job related text', async () => {
  const res = await extractWithAI('Random text', mockModel);
  assert.strictEqual(res, null);
});

test('Hacker News AI Extraction Logic - should handle objects missing required keys gracefully', async () => {
