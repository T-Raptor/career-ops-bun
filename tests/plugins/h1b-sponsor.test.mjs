...
if (!existsSync(TOKEN_PATH)) {
  ...
  const checkArgs = process.versions.bun ? ['build', TOKEN_PATH, '--no-bundle'] : ['--check', TOKEN_PATH];
  if (run(NODE, checkArgs) !== null) pass('token.mjs parses (syntax check)');
  else fail('token.mjs failed syntax check');
