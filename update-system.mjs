 ...
 *   bun update-system.mjs apply --force
  ...
  try {
    ...
    if (atRisk.length > 0) {
      ...
      if (updateForce) {
        ...
        console.log('Re-run with `bun update-system.mjs apply --force` to take the upstream version instead.');
      }
      console.log('');
    }
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  try {
    switch (cmd) {
      ...
      default:
        console.log('Usage: bun update-system.mjs [check|apply [--force]|rollback|dismiss]');
        process.exit(1);
    }
  } catch (err) {
