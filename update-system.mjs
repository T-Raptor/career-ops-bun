 ...
 *   bun update-system.mjs apply --confirm
 *                                     # Apply update after explicit confirmation
 *   bun update-system.mjs apply --force --confirm
  ...
  try {
    ...
    if (atRisk.length > 0) {
      ...
      if (updateForce) {
        ...
        console.log('Re-run with `bun update-system.mjs apply --force --confirm` to take the upstream version instead.');
    ...
    if (unmaterialized.length > 0) {
      ...
      console.error('Run `bun update-system.mjs apply --confirm` again — the updater itself is now current,');
...
if (isCli) {
  ...
  try {
    switch (cmd) {
      ...
      default:
        console.log('Usage: bun update-system.mjs [check|apply --confirm [--force]|rollback|dismiss]');
