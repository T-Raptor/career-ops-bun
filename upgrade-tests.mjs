 ...
 *
 * Usage:
 *   bun upgrade-tests.mjs --pr-gate     # newest old tag -> HEAD, one leg per
 *                                        # dismiss-marker state (untracked/tracked)
 *   bun upgrade-tests.mjs --canary      # planted user-file clobber must go RED
 *   bun upgrade-tests.mjs --local-paths # a declared fork-local path survives (#2421)
...
if (isMainModule(import.meta.url)) {
  ...
  else { console.error('Usage: bun upgrade-tests.mjs --pr-gate | --canary | --local-paths'); process.exit(1); }
}
